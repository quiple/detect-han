...
3. 한 언어에만 속하는 레코드만 델타·가변 길이 방식으로 압축해 `src/generated/unihan-core.ts`를 다시 만듭니다.
...
- 코드 포인트 차이와 7비트 지역 마스크를 가변 길이 정수로 묶어 저장합니다.
- ASCII나 한자가 없는 입력에서는 데이터 테이블을 만들지 않습니다.
- 첫 한자 조회 시에만 정렬된 `Uint32Array`로 복원하고 이후에는 이진 검색합니다.
- 문자열을 별도 배열로 펼치지 않고 UTF-16을 한 번 순회하므로 보조 평면 한자도 추가 할당 없이 처리합니다.

현재 포함된 데이터는 Unicode 17.0.0 기준 14,313개 레코드이며, 복원된 검색 테이블은 57,252바이트입니다.
현재 ESM 빌드는 약 38.6KiB(최대 gzip 압축 시 약 13.3KiB)입니다. 데이터 갱신에 따라 크기는 조금 달라질 수 있습니다.
